...
class DailyTrading(BaseModel):
    ...
    def update(cls):
        ...
        batch_size = 200  # 每10个股票的数据作为一批次处理

        # 添加进度条
        with tqdm(total=len(stock_list), desc="更新当日股票数据") as pbar:
            for idx, row in stock_list.iterrows():
                df = collector.get_stock_data(stock_code=row['code'])
                if not df.empty:
    ...
    def update_all(cls):
        ...
        with tqdm(total=len(stock_list), desc="更新历史股票数据") as pbar:
            for idx, row in stock_list.iterrows():
                # time.sleep(2)  # 暂停2秒
                df = collector.get_stock_data(stock_code=row['code'], start_date='20220101', end_date='20241211', stock_name=row['name'])
                if not df.empty:
